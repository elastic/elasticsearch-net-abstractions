...
namespace Elastic.Xunit.Xunit
{
	class TestFrameworkExecutor : XunitTestFrameworkExecutor
	{
		...
		{
			if (TestConfiguration.SeenDeprecations.Count == 0) return;

			Console.WriteLine("-------- SEEN DEPRECATIONS");
			foreach (var d in TestConfiguration.SeenDeprecations.Distinct())
				Console.WriteLine(d);
			Console.WriteLine("--------");
		}
		...
		private static void DumpReproduceFilters(TestAssemblyRunner runner)
		{
			...
			if (runningIntegrations)
				sb.Append("integrate ")
					.Append(TestConfiguration.Configuration.ElasticsearchVersion);
