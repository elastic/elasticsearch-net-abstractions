...
namespace Elastic.Xunit.Configuration
{
	public class YamlConfiguration : TestConfigurationBase
	{
		...
		public YamlConfiguration(string configurationFile)
		{
			...
			{
				SourceSerializer = RandomBool("source_serializer", random),
				TypedKeys = RandomBool("typed_keys", random),
				OldConnection = RandomBool("old_connection", random)
			};
		}
		...
		private bool RandomBool(string key, Random randomizer) =>
			_config.TryGetValue($"random_{key}", out var v) ? bool.Parse(v) : randomizer.Next(4000) % 1 == 0;
